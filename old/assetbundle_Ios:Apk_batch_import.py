...
def copy_selective_from_pair(
    ...
    return candidates, applied, skipped, failed
...
def batch_by_pid(
    ...
    for d in donor_dir.rglob("*"):
        ...
        try:
            cand, appl, skipd, failed = copy_selective_from_pair(
                d, t_path, out_path,
    ...
    return total_files, paired, total_candidates, total_applied, total_skipped, failed_global
...
def run_gui():
    ...
    def do_run():
        try:
            ...
            files, paired, cand, appl, skipd, failed = batch_by_pid(
            ...
            lines = []
            lines.append(f"[BATCH] Donor scanned={files} Paired={paired} Candidates={cand} Applied={appl} Skipped={skipd} Failed={len(failed)}")
